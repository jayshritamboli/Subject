...
import { Ipro, Ires } from '../Modals/product';
...
PRODUCT_URL : string = `${this.BASE_URL}/products.json`
...
private newProdSub$ : Subject<Ipro> = new Subject<Ipro>()
  ...
  fetchAllProducts() : Observable<Ipro[]>{
    return this._httpClient.get<any>(this.PRODUCT_URL).pipe(
      map(obj => {
        let arr : Array<Ipro> = []
   ...
   createProd(pro : Ipro) : Observable<Ires>{
    return this._httpClient.post<any>(this.PRODUCT_URL,pro)
  }
